...
$(function(){
	...
	//初始化表格列名
	$('#SLatencyMeasureDataQueryTable').datagrid({       
	    columns:[[    
...
/*单向时延测量数据左侧树形任务列表结构*/
$('#SLatencyMeasureDataQueryTree').tree({
	url:'ajaxGetAllTasksByTaskType.html?taskType=2',
    onClick: function(node){
		m_taskNum = node.id;
		m_taskTargetArrayForGraph.length=0;
    	$.ajax({
    		url:"SLatencyMeasureDataQueryByList.html",
    		type:"POST",
    		data:{"taskNum":node.id},
    		dataType:"json",
    		success:function(data){
    			m_siMeasureRecords = data.siMeasureRecords;
    			var task = data.task;
    			setSLatencyValue(task);/*点击任务后设置相应任务的数值*/
    			
    			$('#SLatencyMeasureDataQueryTable').datagrid('loadData',m_siMeasureRecords);/*点击任务后设置相应任务的测量数据(所有的数据)*/
    			
    			setReportValue(m_siMeasureRecords);//设置丢包率、往返时延、时延抖动的统计值
    			//loadDataByGraph();
 ...
 * */
function setReportValue(siMeasureRecords)
{
	...
	var length = siMeasureRecords.length;
	if(siMeasureRecords.length != 0)//测量记录不为空
	{
		maxAvgDelay = siMeasureRecords[0].avgDelay;
		maxDelayJitter = siMeasureRecords[0].delayJitter;
		maxPktLoss = siMeasureRecords[0].pktLoss;
		
		minAvgDelay = maxAvgDelay;
		minDelayJitter = maxDelayJitter;
		minPktLoss = maxPktLoss;
		
		for(var i=0;i<siMeasureRecords.length;i++)
		{
			var avgDelay = siMeasureRecords[i].avgDelay;
			var delayJitter = siMeasureRecords[i].delayJitter;
			var pktLoss = siMeasureRecords[i].pktLoss;
...
/*点击任务后设置相应任务的数值*/
function setSLatencyValue(task)
{
...
function onSelectDateTime(d)
{
	...
	$.ajax({
		type:"POST",
		url:"satisfiedSLatencyMeasureDataQueryByList.html",
		data:{"startTestTime":m_beginDate,"endTestTime":m_endDate,"taskNum":m_taskNum,"taskTargetIdString":m_selectValueString},
		success:function(data)
		{
			console.log(data);
			$('#SLatencyMeasureDataQueryTable').datagrid('loadData',data);//加载双向时延测量数据
		},
		error:function()
		{
			console.log("查询条件失败");
...
function checkAll()
{
	...
	$('#SLatencyMeasureDataQueryTable').datagrid('loadData',m_siMeasureRecords);/*点击任务后设置相应任务的测量数据(所有的数据)*/
	//setvalue(siMeasureRecordDatas);//设置显示的值
}
...
function getAllSelect()
{
	...
	$.ajax({
		...
		{
			console.log(data);
			$('#SLatencyMeasureDataQueryTable').datagrid('loadData',data);//加载数据
		},
		error:function()
		{
	...
	{
		$.each(siMeasureRecordDatas, function(i, item) {
			if(item.testTarget == selectValue[i1])
				loadData.push(item);
		});
	}
		
	$('#SLatencyMeasureDataQueryTable').datagrid('loadData',loaddata);//加载数据
	//setvalue(loaddata);*/
}
