 ...
 * 初始化服务响应时间、页面下载时间结构
 */
function initResponseDelayAndFileDelay()
{
	//服务响应时间视图
	//路径配置
    require.config({
  ...
  //文件下载时间视图
    ...
    require(
            ...
            function(ec){
                //基于准备好的dom，初始化echarts图表
                var dynamicViewForFileDelay = ec.init(document.getElementById("dynamicViewForFileDelay"));
                ...
                timeTicket = setInterval(function (){
                    ...
                    // 动态数据接口 addData
                    dynamicViewForFileDelay.addData([
                        [
                            0,        // 系列索引
                            Math.round(Math.random() * 1000), // 新增数据
                            false,     // 新增数据是否从队列头部插入
                ...
                dynamicViewForFileDelay.setOption(option);
            }
    );
