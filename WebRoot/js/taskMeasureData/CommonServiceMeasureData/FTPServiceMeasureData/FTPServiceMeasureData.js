...
$(function(){
	...
	//3. 通用业务性能测量数据-ftp业务类型的任务列表初始化
	$('#FTPCommonServiceMeasureDataQueryTree').tree({
		url:'ajaxGetAllTasksByTaskType.html?taskType=8&&serviceType=ftp',
		...
		{	
			...
			$.ajax({
				...
				{
					setFTPServiceValue(data.task);//选中任务后，设置通用业务性能任务-ftp的数值
					$('#FTPCommonServiceMeasureDataQueryTree').datagrid('loadData',[]);//加载测量数据
				},
				error:function()
				{
 ...
 * 设置通用业务性能测量任务的数值
 */
function setFTPServiceValue(task)
{
	//1. 设置任务名称
