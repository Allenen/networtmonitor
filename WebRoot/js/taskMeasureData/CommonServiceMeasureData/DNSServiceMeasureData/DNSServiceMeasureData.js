...
$(function(){
	...
	//3. 通用业务性能测量数据-DNS业务类型的任务列表初始化
	$('#DNSCommonServiceMeasureDataQueryTree').tree({
		url:'ajaxGetAllTasksByTaskType.html?taskType=8&&serviceType=dns',
		...
		{	
			...
			$.ajax({
				...
				{
					setDNSServiceValue(data.task);//选中任务后，设置通用业务性能任务-DNS的数值
					$('#DNSCommonServiceMeasureDataQueryTree').datagrid('loadData',[]);//加载测量数据
				},
				error:function()
				{
 ...
 * 设置通用业务性能测量任务的数值
 */
function setDNSServiceValue(task)
{
	//1. 设置任务名称
