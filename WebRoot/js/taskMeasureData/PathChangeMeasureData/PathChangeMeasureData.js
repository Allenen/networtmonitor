...
/**
 * 链路变化测量数据的js文件 by zxgm 2016-10-31
 * */
 
 var m_taskNum = undefined, m_taskTargetArrayForGraph=[], m_pathChangeMeasureRecords = undefined;
/**
 * 文档加载完成后触发
 */
$(function(){
	...
	$('#PathChangeMeasureDataQueryTree').tree({
		url:'ajaxGetAllTasksByTaskType.html?taskType=6',
		onClick: function(node)
		{
			...
			$.ajax({
				...
				{
					setPathChangeValue(data.task);//点击任务后设置相应任务的数值
					$('#PathChangeMeasureDataQueryTable').datagrid('loadData',[]);//加载双向时延测量数据
