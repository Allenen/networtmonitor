 ...
 */
function showSatisfiedSLatencyMeasureData()
{
	//1. 得到选中的任务号
	...
	$.ajax({
		...
		{
			m_siMeasureRecords = data;
			$('#SLatencyMeasureDataQueryTable').datagrid('loadData',data);//加载双向时延测量数据
			if(m_siMeasureRecords != null)
				setReportValue(m_siMeasureRecords);//5. 设置丢包率、往返时延和时延抖动的值
		},
		error:function()
		{
...
function checkAll()
{
	...
	$('#SLatencyMeasureDataQueryTable').datagrid('loadData',m_biMeasureRecords);/*点击任务后设置相应任务的测量数据(所有的数据)*/
}
...
function getAllSelect()
{
	...
	showSatisfiedSLatencyMeasureData();
}
