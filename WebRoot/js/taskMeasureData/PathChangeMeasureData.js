...
var m_pathChangeMeasureRecords;//全局变量存储所有的测量数据
...
/*双向时延测量数据左侧树形任务列表结构*/
$('#PathChangeMeasureDataQueryTree').tree({
	url:'ajaxGetAllTasksByTaskType.html?taskType=5',
    onClick: function(node){
		m_taskNum = node.id;
		m_taskTargetArrayForGraph.length=0;
    	$.ajax({
    		url:"pathChangeMeasureDataQueryByList.html",
    		type:"POST",
    		data:{"taskNum":node.id},
    		dataType:"json",
    		success:function(data){
    			m_pathChangeMeasureRecords = data.pathChangeMeasureRecords;
    			var task = data.task;
    			setPathChangeValue(task);/*点击任务后设置相应任务的数值*/
    			
    			$('#PathChangeMeasureDataQueryTable').datagrid('loadData',m_pathChangeMeasureRecords);/*点击任务后设置相应任务的测量数据(所有的数据)*/
    			
    			setReportValue(m_pathChangeMeasureRecords);//设置丢包率、往返时延、时延抖动的统计值
    		},
    		error:function(){
    			console.log("加载链路变化测量数据失败");
    		}
    	});
 	}
 ...
 * */
function setReportValue(pathChangeMeasureRecords)
{
	...
	var length = pathChangeMeasureRecords.length;
	if(pathChangeMeasureRecords.length != 0)//测量记录不为空
	{
		maxAvgDelay = pathChangeMeasureRecords[0].avgDelay;
		maxDelayJitter = pathChangeMeasureRecords[0].delayJitter;
		maxPktLoss = pathChangeMeasureRecords[0].pktLoss;
		
		minAvgDelay = maxAvgDelay;
		minDelayJitter = maxDelayJitter;
		minPktLoss = maxPktLoss;
		
		for(var i=0;i<pathChangeMeasureRecords.length;i++)
		{
			var avgDelay = pathChangeMeasureRecords[i].avgDelay;
			var delayJitter = pathChangeMeasureRecords[i].delayJitter;
			var pktLoss = pathChangeMeasureRecords[i].pktLoss;
...
/*点击任务后设置相应任务的数值*/
function setPathChangeValue(task)
{
...
function onSelectDateTime(d)
{
	...
	$.ajax({
		type:"POST",
		url:"satisfiedPathChangeMeasureDataQueryByList.html",
		data:{"startTestTime":m_beginDate,"endTestTime":m_endDate,"taskNum":m_taskNum,"taskTargetIdString":m_selectValueString},
		success:function(data)
		{
			console.log(data);
			$('#PathChangeMeasureDataQueryTable').datagrid('loadData',data);//加载双向时延测量数据
		},
		error:function()
		{
			console.log("查询条件失败");
...
function getAllSelect()
{
	...
	$.ajax({
		...
		{
			console.log(data);
			$('#PathChangeMeasureDataQueryTable').datagrid('loadData',data);//加载数据
		},
		error:function()
		{
