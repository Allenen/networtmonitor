...
{
	//1.表单数据序列化为对象
	var jsonObj = $("#DLatencyForm").serializeObject();
	//2.提示输入策略名
	var policy={};
...
function saveTask(isActive,taskNum)
{
	...
	task_target = getChooseTargetEquips("#DLatencyChooseTargetDevices");
	
    //4.获取策略信息
    var choosePolicyName = $("#DLatencyTaskAddUIStrategy").combobox("getValue");//得到策略的名称
    $.ajax(
    {
    	type:"POST",
 ...
 * 初始化双向时延UI
 */
function initDLatencyTaskAddUI()
{
	//1.1设置缺省任务名
	$("#DLatencyTaskName").textbox("setValue", DLatencyTaskName+"任务"+new Date().toLocaleString());
	
	//1.2任务主体的数据初始化
	$("#DLatencyTask_subject").combobox(
	{
		...
		{
			var data = $("#DLatencyTask_subject").combobox('getData');
			$("#DLatencyTask_subject").combobox('select', data[0].text);
		},
		error:function()
		{
			alert("任务主体数据初始化失败");
	...
	//1.3策略参数的选择策略下拉数据初始化
	$("#DLatencyTaskAddUIStrategy").combobox(
	{
		url:'getAjaxPolicyBytaskType.html?tasktype=1',
	...
	{   
		...
		success: function(data)
		{
			$.each(data.data, function(i,item)
			{
				...
				$("#DLatencyTargetDevices").append(optionContent);
			});
		}
	});
