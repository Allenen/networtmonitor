...
$(document).ready(function(){
	$.fn.zTree.init($("#pathPerformanceTree"), setting);
});


var PathPerformanceTaskName="链路性能监测";
...
{
	var jsonobj = $("#PathPerformanceForm").serializeObject();
	//1.判断是否保存编辑任务
 ...
 * 初始化双向时延UI
 */
function initPathPerformanceTaskAddUI()
{
	//1.1设置缺省任务名
	$("#PathPerformanceTaskName").textbox("setValue", PathPerformanceTaskName+"任务"+new Date().toLocaleString());
	
	//1.2任务主体的数据初始化
	$("#PathPerformanceTask_subject").combobox(
	{
		...
		{
			var data = $("#PathPerformanceTask_subject").combobox('getData');
			$("#PathPerformanceTask_subject").combobox('select', data[0].value);
		},
		error:function()
		{
			alert("任务主体数据初始化失败");
...
{
	console.log($("#diyInput1"));
	console.log($("#diyInput5").attr("value"));
	console.log($("#diyInput6"));
	console.log($("#diyInput7").attr("value"));
	
	var deviceNodeIds=[],interfaceNodeIds=[],equipList=[],policypara="";
	var treeObj = $.fn.zTree.getZTreeObj("pathPerformanceTree");
