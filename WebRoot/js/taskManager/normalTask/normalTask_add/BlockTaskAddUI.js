...
{
	//1.表单数据序列化为对象
	var jsonObj = $("#BlockForm").serializeObject();
	//2.提示输入策略名
	var policy={};
...
function saveTask(isActive,taskNum)
{
	...
	task_target = getChooseTargetEquips("#BlockChooseTargetDevices");
	
    //4.获取策略信息
    var choosePolicyName = $("#BlockTaskAddUIStrategy").combobox("getValue");//得到策略的名称
    $.ajax(
    {
    	type:"POST",
 ...
 * 初始化双向时延UI
 */
function initBlockTaskAddUI()
{
	//1.1设置缺省任务名
	$("#BlockTaskName").textbox("setValue", BlockTaskName+"任务"+new Date().toLocaleString());
	
	//1.2任务主体的数据初始化
	$("#BlockTask_subject").combobox(
	{
		...
		{
			var data = $("#BlockTask_subject").combobox('getData');
			$("#BlockTask_subject").combobox('select', data[0].text);
		},
		error:function()
		{
			alert("任务主体数据初始化失败");
	...
	//1.3策略参数的选择策略下拉数据初始化
	$("#BlockTaskAddUIStrategy").combobox(
	{
		url:'getAjaxPolicyBytaskType.html?tasktype=3',
	...
	{   
		...
		success: function(data)
		{
			$.each(data.data, function(i,item)
			{
				...
				$("#BlockTargetDevices").append(optionContent);
			});
		}
	});
